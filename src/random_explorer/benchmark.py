...
class Benchmark:
    ...
    def run_multi_robot_rrt(
        ...
        planner = MultiRobotRRTPlanner(
            env=env,
            max_iter=final_params['max_iter'],
            delta_s=final_params['delta_s'],
            delta_r=final_params['delta_r'],
            goal_bias=final_params.get('goal_bias', 0.05),
            goal_tolerance=final_params.get('goal_tolerance', 10.0)
        )
        ...
        for idx, (name, result) in enumerate(results.items()):
            ...
            for i, (xo, yo, lx, ly) in enumerate(env.obstacles):
                rect = patches.Rectangle(
                    ...
                    label='Obstacles' if i == 0 and idx == 0 else None
                )
                ax.add_patch(rect)
            ...
            ax.plot(env.goal[0], env.goal[1], 'g^', markersize=8,
                    markeredgecolor='darkgreen', markeredgewidth=1.5,
            ...
            ax.plot(env.goal2[0], env.goal2[1], 'b^', markersize=8,
                    markeredgecolor='darkblue', markeredgewidth=1.5,
